...
namespace BookLib
{
    public class ItemCollection : IEnumerable, IEnumerable<AbstractItem>, ICustomer<AbstractItem>, IWorker<AbstractItem>
    {
        ...
        bool Remove(AbstractItem item, bool isAll, int amount = 0)
        {
            ...
            //reports.Add(new Report(ReportType.Remove, prod.Item, count));
            return true;
        }
        public bool Borrow(AbstractItem item)
        {
            ...
            a.InStock--;
            //reports.Add(new Report(ReportType.borrow, item, 1));
            return true;
        }
        public bool Buy(AbstractItem item)
        {
            ...
            a.Amount--;
            a.InStock--;
            //reports.Add(new Report(ReportType.buy, a.Item, 1));
            return true;
        }
        public int Count
        {
        ...
        public ObservableCollection<AbstractItem> ShowAll()
        {
            var lst = new ObservableCollection<AbstractItem>();
        ...
        public ObservableCollection<AbstractItem> FilterBy(Filter filter, double min, double max)
        {
            ObservableCollection<AbstractItem > lst;
            switch (filter)
            {
                case Filter.Available:
                    lst = new ObservableCollection<AbstractItem>(ShowAll()
                        .Where(x => AvailableAmountOf(x) >= min && AvailableAmountOf(x) <= max));
                    break;
                case Filter.Amount:
                    lst = new ObservableCollection<AbstractItem>(ShowAll()
                        .Where(x => AmountOf(x) >= min && AmountOf(x) <= max));
                    break;
                case Filter.Price:
                default:
                    lst = new ObservableCollection<AbstractItem>(ShowAll()
                        .Where(x => PriceOf(x) >= min && PriceOf(x) <= max));
                    break;
            }
            return lst;
        }
